...
//		try {
//			String redirectUrl = getSuccessfulRedirect(authorizationRequest,
//					generateCode(authorizationRequest, authUser));
//			redirectUrl = "/oauth/authorize/finish/" + URLUtil.encodeAll(redirectUrl, StandardCharsets.UTF_8);
//
...
//	}
	...
	private View getAuthorizationCodeResponse(AuthorizationRequest authorizationRequest, Authentication authUser) {
		try {
			Rest<BaseResponse> response = Rest.vector("redirectUrl", getSuccessfulRedirect(authorizationRequest,
					generateCode(authorizationRequest, authUser)), String.class);
			HttpContextUtil.write(HttpContextUtil.getHttpServletResponse(), response);
